    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(print_reply(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(print_reply(second, Arc::clone(&reply)));

    tokio::join!(handle1, handle2);
}

async fn print_reply<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
